---
tags:
  - Entities
show:
  - toc
alias:
  - blue_mountains_miner
---

####

::infobox
type: entity
title: Blue Mountains Miner
image: entities/blue_mountains_miner.png
armor: 0
hitpoints: 20
faction: [[blue_mountains_faction|Blue Mountains]]
::end-infobox

# Blue Mountains Miner

Blue Mountain Miners exclusively spawn naturally in the [[blue_mountains_faction|Blue Mountains]]. Unlike most other traders, Miners will naturally despawn according the despawn logic Minecraft uses for entities.
Blue Mountain Miners are usually found in the caves, and all structures that generate under the ground in Blue Mountains. They carry carry a Blue Dwarven Pickaxe and a silver coin.

## Behavior

Miners will automatically attack any enemies, fighting back against attackers with their pickaxe.

## Equipment

Blue Mountains Miners wear no armour, making them an decently easy NPC to kill. Although they will charge you without being provoked, unlike a [[blue_mountains_commander|Commander]] or [[blue_mountains_merchant|Merchant]].

## Trading

Miners will not trade with players unless they have +100 alignment with [[blue_mountain_faction|Blue Mountains]]. Upon trading with then, the player will be bestowed the advancement "**Blue Riches**".
Blue Mountain Miners sell their mining spoilers from their diggings. This includes gemstones, raw ores, powders, coal and glowstone dust.
Blue Mountain Miners buy supplies for their mining adventures. They primarily buy cooked meats such as duck, mutton, porkchops, as well as bread and gammon. They also buy new pickaxes so to replenish their worn ones. 

## Strategy

- There is no need to fight a Miner, but if you do, it is easy to kill them with a longer weapon such as a spear, or with a bow, since they don't have a ranged attack and they have shorter range due to being a dwarf.

## Lore

The Blue Mountains Miners dwell deep in their mountains of their homeland Ered Luin, building and mining passageways far underground.

## History
- 1.8.0:
    - Ported Blue Mountains Miner

## Trivia

- Blue Mountain Miners were the first naturally spawning non-permanent trader added.
